...
def main():
    ...
    loader = DataLoader(ds, batch_size=1, num_workers=cfg["num_workers"], pin_memory=True)
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="Predict", ncols=100):
            ...
            # 还原 case_id
            case_id = Path(batch["image_meta_dict"]["filename_or_obj"][0]).parent.name \
                if "image_meta_dict" in batch else Path(files[loader.dataset.data.index(batch)]["image"][0]).parent.name
