#!/usr/bin/env python3
"""生成论文 Table 1: Dice / HD95 / Sensitivity / Specificity / Precision per region per model."""

import argparse, time
from pathlib import Path
import numpy as np
import nibabel as nib
from joblib import Parallel, delayed


REGIONS = ["tc", "wt", "et"]


def gt_regions(seg):
    seg = seg.astype(np.uint8)
    return {"tc":(seg==1)|(seg==4), "wt":seg>0, "et":seg==4}


def load_prob(p):
    z = np.load(p); pr = z["prob"]
    if pr.dtype != np.uint8:
        pr = (pr*255 if pr.max()<=1 else pr).clip(0,255).astype(np.uint8)
    return pr


def ensemble(probs, weights):
    out = np.zeros_like(probs[0], dtype=np.float32)
    for p, w in zip(probs, weights):
        out += p.astype(np.float32) * float(w)
    return np.clip(out, 0, 255).astype(np.uint8)


def metrics_binary(pred, gt):
    """Dice, Sensitivity (=Recall=TPR), Specificity (=TNR), Precision (=PPV)."""
    pred = pred.astype(bool); gt = gt.astype(bool)
    tp = (pred & gt).sum()
    fp = (pred & ~gt).sum()
    fn = (~pred & gt).sum()
    tn = (~pred & ~gt).sum()
    dice = 2*tp / (2*tp + fp + fn + 1e-8) if (tp+fp+fn) > 0 else 1.0
    sens = tp / (tp + fn + 1e-8) if (tp+fn) > 0 else 1.0
    spec = tn / (tn + fp + 1e-8) if (tn+fp) > 0 else 1.0
    prec = tp / (tp + fp + 1e-8) if (tp+fp) > 0 else 1.0
    return float(dice), float(sens), float(spec), float(prec)


def process_single(cid, prob_dir, ref_dir, threshold):
    """Single model."""
    try:
        prob = load_prob(prob_dir / f"{cid}.npz")
    except Exception:
        return None
    gp = ref_dir / cid / f"{cid}_seg.nii.gz"
    if not gp.exists(): return None
    seg = np.asarray(nib.load(str(gp)).dataobj)
    gt = gt_regions(seg)
    thr_u8 = int(round(threshold * 255))
    row = {"case": cid}
    for i, r in enumerate(REGIONS):
        pred = prob[i] >= thr_u8
        d, se, sp, p = metrics_binary(pred, gt[r])
        row[f"dice_{r}"] = d
        row[f"sens_{r}"] = se
        row[f"spec_{r}"] = sp
        row[f"prec_{r}"] = p
    return row


def process_ensemble(cid, prob_dirs, ref_dir, weights, thresholds):
    try:
        probs = [load_prob(d / f"{cid}.npz") for d in prob_dirs]
    except Exception:
        return None
    gp = ref_dir / cid / f"{cid}_seg.nii.gz"
    if not gp.exists(): return None
    seg = np.asarray(nib.load(str(gp)).dataobj)
    gt = gt_regions(seg)
    ens = ensemble(probs, weights)
    row = {"case": cid}
    for i, r in enumerate(REGIONS):
        pred = ens[i] >= int(round(thresholds[r] * 255))
        d, se, sp, p = metrics_binary(pred, gt[r])
        row[f"dice_{r}"] = d
        row[f"sens_{r}"] = se
        row[f"spec_{r}"] = sp
        row[f"prec_{r}"] = p
    return row


def main():
    import json
    cfg = json.loads(Path("work_dir/grid_search/best_config.json").read_text())
    weights_dict = cfg["weights"]
    thresholds = cfg["thresholds"]
    
    ref_dir = Path("data/BraTS2021_TrainingData")
    
    # Models
    results = {}
    
    for name, path in [("segresnet", "/dev/shm/brats_probs/segresnet_oof"),
                       ("swinunetr", "/dev/shm/brats_probs/swinunetr_oof")]:
        print(f"\n═══ {name} (single, OOF) ═══")
        prob_dir = Path(path)
        cases = sorted([f.stem for f in prob_dir.glob("*.npz") if not f.name.startswith("_tmp_")])
        t0 = time.time()
        rows = Parallel(n_jobs=-1, verbose=5)(
            delayed(process_single)(c, prob_dir, ref_dir, 0.5) for c in cases
        )
        rows = [r for r in rows if r]
        print(f"  {len(rows)} cases, {time.time()-t0:.0f}s")
        results[name] = rows
    
    # Ensemble
    print(f"\n═══ ensemble (best config) ═══")
    prob_dirs = [Path("/dev/shm/brats_probs/segresnet_oof"),
                 Path("/dev/shm/brats_probs/swinunetr_oof")]
    weights = [weights_dict["segresnet"], weights_dict["swinunetr"]]
    cases = sorted([f.stem for f in prob_dirs[0].glob("*.npz") if not f.name.startswith("_tmp_")])
    t0 = time.time()
    rows = Parallel(n_jobs=-1, verbose=5)(
        delayed(process_ensemble)(c, prob_dirs, ref_dir, weights, thresholds) for c in cases
    )
    rows = [r for r in rows if r]
    print(f"  {len(rows)} cases, {time.time()-t0:.0f}s")
    results["ensemble"] = rows

    # Print table
    print()
    print("="*100)
    print("📋 论文 Table 1: 各模型各区域指标 (mean ± std)")
    print("="*100)
    
    header = f"{'Model':<12} {'Region':<8}"
    for m in ["Dice", "Sensitivity", "Specificity", "Precision"]:
        header += f" {m:>14}"
    print(header)
    print("-" * 100)
    
    table_rows = []
    for model_name in ["segresnet", "swinunetr", "ensemble"]:
        rows = results[model_name]
        for r in REGIONS:
            d = np.array([row[f"dice_{r}"] for row in rows])
            se = np.array([row[f"sens_{r}"] for row in rows])
            sp = np.array([row[f"spec_{r}"] for row in rows])
            pr = np.array([row[f"prec_{r}"] for row in rows])
            line = f"{model_name:<12} {r.upper():<8}"
            for vals in [d, se, sp, pr]:
                line += f"  {vals.mean():.4f}±{vals.std():.3f}"
            print(line)
            table_rows.append({
                "model": model_name, "region": r.upper(),
                "dice": f"{d.mean():.4f}±{d.std():.3f}",
                "sensitivity": f"{se.mean():.4f}±{se.std():.3f}",
                "specificity": f"{sp.mean():.4f}±{sp.std():.3f}",
                "precision": f"{pr.mean():.4f}±{pr.std():.3f}",
            })
        # Mean row
        d_m = np.mean([[row[f"dice_{r}"] for r in REGIONS] for row in rows], axis=1)
        se_m = np.mean([[row[f"sens_{r}"] for r in REGIONS] for row in rows], axis=1)
        sp_m = np.mean([[row[f"spec_{r}"] for r in REGIONS] for row in rows], axis=1)
        pr_m = np.mean([[row[f"prec_{r}"] for r in REGIONS] for row in rows], axis=1)
        line = f"{model_name:<12} {'Mean':<8}"
        for vals in [d_m, se_m, sp_m, pr_m]:
            line += f"  {vals.mean():.4f}±{vals.std():.3f}"
        print(line)
        print()

    # Save CSV
    import pandas as pd
    out = Path("paper_data/csv/paper_table1.csv")
    out.parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(table_rows).to_csv(out, index=False)
    print(f"✓ saved CSV: {out}")


if __name__ == "__main__":
    main()
