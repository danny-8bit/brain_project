...
def process_case_thresholds(cid, prob_dirs, ref_dir, weights, triples, et_threshold_voxels, et_min_prob):
    ...
    for tc_t, wt_t, et_t in triples:
        wt_mask = wt_cache.setdefault(wt_t, wt_prob >= wt_t)
        raw_tc = tc_cache.setdefault(tc_t, tc_prob >= tc_t)
        tc_mask = raw_tc & wt_mask
        raw_et = et_cache.setdefault(et_t, et_prob >= et_t)
        ...
        result[(tc_t, wt_t, et_t)] = score_masks(tc_mask, wt_mask, et_mask, gt)
    return cid, result
